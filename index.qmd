...
```{python}
#| code-line-numbers: "1-6|8-9|11-14|16-19"
import numpy as np
...
**Substitute the particle approximation** $\rho^{\mathrm{vol}}\approx\rho_N = \sum_{i=1}^N w_i\,\delta(\mathbf{x}-\mathbf{x}^i(t))\qquad$
and use $\mathbf{u}(t,\mathbf{x}^i(t))=\dot{\mathbf{x}}^i(t)=:\mathbf{v}_i(t)\,\,\,$:
...
where $\rho_i := \rho_{h,N}(t,\mathbf{x}^i) = \sum_j w_j\,W_h(\mathbf{x}^i-\mathbf{x}^j)\quad$.
...
::: {style="margin-top: -1em;"}
**Isothermal:** $\mathcal{U}(\rho) = \kappa\,\ln\rho\,\,\,$, $\;\kappa\,\,\,$ constant $\;\Rightarrow\;p(\rho) = \kappa\rho$
...
**Polytropic:** $\mathcal{U}(\rho) = \dfrac{\kappa\,\rho^{\gamma-1}}{\gamma-1}\,\,\,$, $\;\kappa\,\,\,$ polytropic constant, $\;\gamma = C_p/C_v\,\,\,$ adiabatic index $\;\Rightarrow\;p(\rho) = \kappa\rho^\gamma$
...
where $(\mathbf{a}\otimes\mathbf{b})_{jk}=a_j b_k\,\,\,$ is the outer product, so that $(\nabla\mathbf{v})_{jk}=\partial_j v_k\,$.
...
where $\boldsymbol\sigma(\boldsymbol\eta_l)\,\nabla W_h\,\,\,$ is the **matrix–vector product** of the $3\times3$ stress tensor at $\boldsymbol\eta_l$ with the kernel gradient.

::: {style="margin-top: -0.5em;"}
**Algorithmic structure**

1. **Precompute:** evaluate $\rho^{N,h}(\boldsymbol\eta_i)$ and $\boldsymbol\sigma(\boldsymbol\eta_i)$ at each particle; store the 9 weighted coefficients $-w_i\,\boldsymbol\sigma(\boldsymbol\eta_i)/\rho^{N,h}(\boldsymbol\eta_i)$
2. **Evaluate:** sum over neighbors — same neighbor-loop structure as the **pressure force**, but replacing scalar weights by tensor coefficients
3. **Note:** the pair-wise antisymmetry of the pressure force does _not_ carry over automatically — momentum conservation requires careful numerical treatment
:::
