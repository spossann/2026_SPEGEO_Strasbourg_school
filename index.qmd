...
- [4] J. Bonet et al., "Variational formulation for the smooth particle hydrodynamics (SPH) simulation of fluid and solid problems", Comp. Meth. in App. Mech. and Eng., Volume 193, Issues 12–14, 2004.
...
8. _Struphy: an open-source Python package for solving PDEs_
  ...
  - Built on top of [Pyccel](https://github.com/pyccel/pyccel) for high-performance kernel execution
...
$$
\mathcal A[\mathbf x] = \int_{t_0}^{t_1} \left( \frac{m}{2}|\dot{\mathbf x}|^2 - V\right)\,\mathrm dt,\qquad \delta \mathcal A = 0,
\qquad \Rightarrow \quad \frac{\mathrm d}{\mathrm dt}\frac{\partial L}{\partial \dot{\mathbf x}} - \frac{\partial L}{\partial \mathbf x} = 0
$$
...
x  = 0.30 + 0.15*np.sin(np.pi*t) + 0.04*np.sin(2*np.pi*t)
...
&= \ddot{\mathbf x} \cdot \frac{\partial L}{\partial \dot{\mathbf x}} + \dot{\mathbf x} \cdot \frac{\mathrm d}{\mathrm dt} \frac{\partial L}{\partial \dot{\mathbf x}} - \dot{\mathbf x} \cdot \frac{\partial L}{\partial \mathbf x} - \ddot{\mathbf x} \cdot \frac{\partial L}{\partial \dot{\mathbf x}} - \frac{\partial L}{\partial t}
\\
...
\qquad\Longrightarrow\qquad
\frac{\mathrm d}{\mathrm dt}\left(\frac{\partial L}{\partial \dot{\mathbf x}}\right)=0\,,\qquad p := \frac{\partial L}{\partial \dot{\mathbf x}},
...
Let $\mathbf u(t,\mathbf x)\quad$ be the _fluid velocity field_ at any given time $t$ at position $\mathbf x$.
...
fig, (axL, axE) = plt.subplots(1, 2, figsize=(9, 2.8))
...
fig, ax = plt.subplots(figsize=(8.976, 3.344))
...
$$
\begin{aligned}
	\text{①} = \int_0^T \int_{\Omega} \mathbf u \cdot \delta \mathbf u \, \rho^\textrm{vol} \, \textrm d \mathbf x\,\mathrm dt &= \int_0^T \int_{\Omega} \mathbf u \cdot \left( \partial_t \boldsymbol\eta + \mathbf u \cdot \nabla \boldsymbol\eta - \boldsymbol\eta \cdot \nabla \mathbf u \right) \rho^\textrm{vol} \, \textrm d \mathbf x\,\mathrm dt
  \\[2mm]
...
$$
\begin{aligned}
  \text{②} = \int_0^T \int_{\Omega} \left( \frac{|\mathbf u|^2}{2} - \frac 1m V \right) \delta \rho^\textrm{vol} \, \textrm d \mathbf x\,\mathrm dt &= - \int_0^T \int_{\Omega} \left( \frac{|\mathbf u|^2}{2} - \frac 1m V \right) \nabla \cdot (\rho^\textrm{vol} \boldsymbol\eta) \, \textrm d \mathbf x\,\mathrm dt
  \\[2mm]
...
fig, ax = plt.subplots(figsize=(11.28, 3.36))
...
1. Define domain, background density, and perturbation
2. Create a `ParticlesSPH` object and draw markers
3. Initialize weights $w_i$ from the background density
4. Evaluate the SPH density estimate $\rho_{h,N}(\eta_1) = \sum_i w_i\,W_h(\eta_1 - \eta_1^i)$
5. Compare with the exact solution

The corresponding test is `test_sph_evaluation_1d` in
`struphy/src/struphy/pic/tests/test_sph.py`.
...
eta1 = np.linspace(0, 1.0, 300)
rho_exact = 1.5 + np.cos(2 * np.pi * eta1)

fig, ax = plt.subplots(figsize=(10, 2.8))
ax.plot(eta1, rho_exact, color='#0f4c81', lw=2.5,
        label=r'$\rho(\eta_1) = 1.5 + \cos(2\pi\eta_1)$')
ax.fill_between(eta1, 0, rho_exact, alpha=0.10, color='#0f4c81')
ax.set_xlabel(r'$\eta_1$', fontsize=13)
...
```{python}
#| code-line-numbers: "1-6|8-9|11-14|16-18|19-22"
import numpy as np
...
sorting_params  = SortingParameters(boxes_per_dim=(24, 1, 1))
```


## Step 2: Draw markers and initialize weights

```{python}
#| code-line-numbers: "1-11|13-14"
...
particles.initialize_weights()      # compute w_i from background density
```

. . .

- `draw_markers` places one marker per cell center (tesselation) or draws random samples
- `initialize_weights` sets $w_i = \int_{\Omega_0^i} \rho^\text{vol}(0, \mathbf x_0)\,\mathrm d\mathbf x_0$


## Step 3: Evaluate the SPH density

```{python}
#| code-line-numbers: "2-3|5-6|8-13|15-17"
# evaluation grid
...
print(f"Max relative error: {err:.4f}")
```
...
ax.plot(eta1, rho_exact, color='#0f4c81', lw=2.5, label='exact')
ax.plot(eta1, np.array(rho_sph).squeeze(), '--o', color='#d95f02', lw=2.0,
        markersize=3, label='SPH (gaussian_1d)')
ax.set_xlabel(r'$\eta_1$', fontsize=13)
ax.set_ylabel(r'$\rho$', fontsize=13)
ax.set_xlim(0, 1)
ax.legend(fontsize=12)
ax.grid(True, alpha=0.25)
ax.set_title(f'Max relative error = {err:.4f}  (tesselation, ppb=4, h=1/24)', fontsize=11)
plt.tight_layout()
plt.show()
```
...
out1, out2, out3 = np.zeros_like(x), np.zeros_like(x), np.zeros_like(x)
...
fig, ax = plt.subplots(figsize=(10, 3.2))
ax.plot(x, out1, color='#1b9e77', lw=2.5, label='trigonometric')
ax.plot(x, out2, color='#d95f02', lw=2.5, label='gaussian')
ax.plot(x, out3, color='#7570b3', lw=2.5, label='linear')
ax.set_xlabel(r'$x/h$', fontsize=13)
ax.set_ylabel(r'$W(x,h)$', fontsize=13)
ax.legend(fontsize=12)
ax.grid(True, alpha=0.25)
plt.tight_layout()
plt.show()
...
:::{style="text-align: center;"}
$$
\rho_{h,N}(\mathbf{x}) = \sum_i w_i\, W_h(\mathbf{x} - \mathbf{x}^i), \qquad
W_h(\mathbf{x}) = \frac{1}{h}\,W\!\left(\frac{|\mathbf{x}|}{h}\right)
$$
:::
...
#| echo: false
#| fig-align: center
kernels = ["gaussian_1d", "trigonometric_1d", "linear_1d"]
...
for ax, kernel, color in zip(axes, kernels, colors):
    rho_k = particles.eval_density(ee1, ee2, ee3, h1=h1, h2=h2, h3=h3,
                                    kernel_type=kernel, derivative=0)
    err_k = np.max(np.abs(np.array(rho_k).squeeze() - rho_exact)) / np.max(np.abs(rho_exact))
    ax.plot(eta1, rho_exact, color='#0f4c81', lw=2.0, label='exact')
    ax.plot(eta1, np.array(rho_k).squeeze(), '--', color=color, lw=2.0, label=kernel)
    ax.set_xlabel(r'$\eta_1$', fontsize=11)
    ax.set_title(f'{kernel}\nerr = {err_k:.4f}', fontsize=11)
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.25)
axes[0].set_ylabel(r'$\rho$', fontsize=13)
plt.suptitle('tesselation, ppb=4, h=1/24', fontsize=11, y=1.01)
plt.tight_layout()
plt.show()
```
...
1. Define domain, background density, and 2D perturbation
2. Create a `ParticlesSPH` object and draw markers on a $12\times12\quad$ grid
3. Initialize weights from the background density
4. Evaluate the 2D SPH density estimate with `gaussian_2d`
5. Compare with the exact solution

The corresponding test is `test_sph_evaluation_2d` in
`struphy/src/struphy/pic/tests/test_sph.py`.
...
#| echo: false
#| fig-align: center
eta1_t = np.linspace(0, 1.0, 200)
eta2_t = np.linspace(0, 1.0, 200)
G1, G2 = np.meshgrid(eta1_t, eta2_t, indexing="ij")
rho_ex_t = 1.5 + np.cos(2 * np.pi * G1) * np.cos(2 * np.pi * G2)

fig, ax = plt.subplots(figsize=(5.0, 3.5))
im = ax.pcolormesh(G1, G2, rho_ex_t, cmap='RdBu_r', shading='auto')
fig.colorbar(im, ax=ax, label=r'$\rho$')
ax.set_xlabel(r'$\eta_1$', fontsize=13)
ax.set_ylabel(r'$\eta_2$', fontsize=13)
plt.tight_layout()
plt.show()
```

$$
\rho(\eta_1,\eta_2) = \underbrace{1.5}_{\text{background}} + \underbrace{\cos(2\pi\eta_1)\cos(2\pi\eta_2)}_{\text{perturbation}},
\qquad (\eta_1,\eta_2)\in[0,1]^2
$$
...
sorting2d  = SortingParameters(boxes_per_dim=(12, 12, 1))
```
...
```{python}
#| code-line-numbers: "1-11|13-14"
particles2d = ParticlesSPH(
    comm_world=None,
    loading_params=loading2d,
    boundary_params=boundary2d,
    sorting_params=sorting2d,
    bufsize=1.0,
    domain=dom2d,
    background=bg2d,
    perturbations=pert2d,
    n_as_volume_form=True,
)

particles2d.draw_markers(sort=False)
particles2d.initialize_weights()
```
...
```{python}
#| code-line-numbers: "2-4|6|8-13|15-17"
# evaluation grid
...
print(f"Max relative error: {err_2d:.4f}")
```
...
sph_arr = np.array(rho_sph_2d).squeeze()
ex_arr  = rho_exact_2d.squeeze()
vmin, vmax = ex_arr.min(), ex_arr.max()
X = ee1_2d.squeeze()
Y = ee2_2d.squeeze()

im0 = axes[0].pcolormesh(X, Y, ex_arr, cmap='RdBu_r', vmin=vmin, vmax=vmax, shading='auto')
axes[0].set_title('Exact', fontsize=13)
axes[0].set_xlabel(r'$\eta_1$', fontsize=12)
axes[0].set_ylabel(r'$\eta_2$', fontsize=12)
fig.colorbar(im0, ax=axes[0])

im1 = axes[1].pcolormesh(X, Y, sph_arr, cmap='RdBu_r', vmin=vmin, vmax=vmax, shading='auto')
axes[1].set_title(f'SPH (gaussian_2d)  —  max rel. err = {err_2d:.4f}', fontsize=13)
axes[1].set_xlabel(r'$\eta_1$', fontsize=12)
...
:::{style="text-align: center;"}
$$
\rho_{h,N}(\eta_1,\eta_2) = \sum_i w_i\, W_h(\eta_1 - \eta_1^i,\,\eta_2 - \eta_2^i)
\approx 1.5 + \cos(2\pi\eta_1)\cos(2\pi\eta_2)
$$
:::
...
**Optimal kernel width** (red curve): trades off the two effects;
as $N\uparrow\,$ the optimal $h$ shrinks — more markers allow a finer kernel.

**Convergence:** along the red curve the minimum error decreases with $N$.
Together $h\to 0\,\,\,$, $N\to\infty\,\,\,\,$ with $N h^d \to\infty$ ensures $\rho_{h,N}\to\rho\qquad$.
:::
:::
...
$$
\mathcal{A}_{\mathrm{Euler}}(\mathbf{u},\rho^{\mathrm{vol}})
= \int_0^T\!\int_{\Omega}
  \!\left(\frac{|\mathbf{u}(t,\mathbf{x})|^2}{2}
         -\frac{V(\mathbf{x})}{m}
         -\mathcal{U}(\rho)\right)
  \rho^{\mathrm{vol}}(t,\mathbf{x})\,\mathrm{d}\mathbf{x}\,\mathrm{d}t
$$

**Substitute the particle approximation** $\rho^{\mathrm{vol}}\approx\rho_N = \sum_{i=1}^N w_i\,\delta(\mathbf{x}-\mathbf{x}^i(t))\qquad\quad\,\,$
and use $\mathbf{u}(t,\mathbf{x}^i(t))=\dot{\mathbf{x}}^i(t)=:\mathbf{v}_i(t)\qquad\,\,\,\,$:

$$
\mathcal{A}_N
= \int_0^T\sum_{i=1}^N w_i
\!\left(\frac{|\mathbf{v}_i(t)|^2}{2}
       -\frac{V(\mathbf{x}^i(t))}{m}
       -\mathcal{U}\!\left(\rho\!\left(t,\mathbf{x}^i(t)\right)\right)\right)\mathrm{d}t
$$

::: {style="margin-top: -0.5em;"}
- The action becomes a **finite-dimensional integral** over particle trajectories
- The term $\mathcal{U}(\rho(t,\mathbf{x}^i))\,\,\,\,\,$ cannot be evaluated with $\rho_N\,\,\,$ (distributional)
  $\Rightarrow\,\,\,$ replace $\rho\,\,$ by a **kernel-smoothed density** $\rho_{h,N}\,\,\,$ (next slide)
:::


## Viscous Euler with SPH: Particle equations

Euler–Lagrange equations of $\mathcal{A}_N\,\,$ (with $\rho_{h,N}\,\,$ replacing $\rho\,\,$)
yield the **SPH particle system**:

$$
\begin{aligned}
    \dot{\mathbf{x}}^i &= \mathbf{v}_i\,,
    \\[3mm]
    \dot{\mathbf{v}}_i &= -\frac{\nabla V(\mathbf{x}^i)}{m}
    \;-\; \sum_{j=1}^N w_j
      \!\left(\frac{\partial\mathcal{U}}{\partial\rho}(\rho_i)+\frac{\partial\mathcal{U}}{\partial\rho}(\rho_j)\right)
...
- **Pressure force**: $-\sum_j w_j\!\left(\tfrac{\partial\mathcal{U}}{\partial\rho}(\rho_i)+\tfrac{\partial\mathcal{U}}{\partial\rho}(\rho_j)\right)\nabla W_h\qquad\,\,\,\,\,$ — the standard _SPH pressure force_, here **derived from a variational principle**
- Pair-wise anti-symmetric form ensures **energy conservation** and exact **linear and angular momentum conservation**
...
$$
\dot{\mathbf{v}}_i = -\frac{\nabla V(\mathbf{x}^i)}{m}
  - \kappa\sum_{j=1}^N w_j
    \!\left(\frac{1}{\rho_i}+\frac{1}{\rho_j}\right)
    \nabla W_h(\mathbf{x}^i-\mathbf{x}^j)
$$
...
\dot{\mathbf{v}}_i = -\frac{\nabla V(\mathbf{x}^i)}{m}
  - \kappa\sum_{j=1}^N w_j
    \!\left(\rho_i^{\gamma-2}+\rho_j^{\gamma-2}\right)
    \nabla W_h(\mathbf{x}^i-\mathbf{x}^j)
$$
:::
...
$$
\nabla\mathbf{v}^{N,h}(\boldsymbol\eta_i)
= \sum_l \frac{w_l}{\rho_{h,N}(\boldsymbol\eta_l)}\;\nabla W_h(\boldsymbol\eta_i - \boldsymbol\eta_l)\otimes\mathbf{v}_l
$$
...
$$
\boldsymbol\sigma(\boldsymbol\eta_i)
=\mu\!\left[
  \nabla\mathbf{v}^{N,h}(\boldsymbol\eta_i)
  +\bigl(\nabla\mathbf{v}^{N,h}(\boldsymbol\eta_i)\bigr)^{\!\top}
  -\tfrac{2}{3}\bigl(\nabla\cdot\mathbf{v}^{N,h}(\boldsymbol\eta_i)\bigr)\,\mathbf{I}
\right]
$$
...
= \sum_l \frac{w_l}{\rho^{N,h}(\boldsymbol\eta_l)}\;\boldsymbol\sigma(\boldsymbol\eta_l)\,\nabla W_h(\boldsymbol\eta_i-\boldsymbol\eta_l)
$$
...
```{=html}
<video width="100%" data-autoplay loop muted playsinline>
  <source src="hagen_poiseuille_mc.mp4" type="video/mp4">
</video>
```
...
```{=html}
<video width="100%" data-autoplay loop muted playsinline>
  <source src="hagen_poiseuille.mp4" type="video/mp4">
</video>
```
...
```{=html}
<video width="100%" data-autoplay loop muted playsinline>
  <source src="dam_break.mp4" type="video/mp4">
</video>
```
...
Available in the [struphy-hub/struphy](https://github.com/struphy-hub/struphy/tree/main/tutorials) repository on GitHub (`tutorials/` folder):

- [`tutorial_beltrami_sph.ipynb`](https://github.com/struphy-hub/struphy/blob/main/tutorials/tutorial_beltrami_sph.ipynb)
- [`tutorial_gas_expansion_sph.ipynb`](https://github.com/struphy-hub/struphy/blob/main/tutorials/tutorial_gas_expansion_sph.ipynb)
- [`tutorial_viscous_euler_sph.ipynb`](https://github.com/struphy-hub/struphy/blob/main/tutorials/tutorial_viscous_euler_sph.ipynb)
- [`tutorial_velocity_diffusion_sph.ipynb`](https://github.com/struphy-hub/struphy/blob/main/tutorials/tutorial_velocity_diffusion_sph.ipynb)
- [`tutorial_hagen_poiseuille_sph.ipynb`](https://github.com/struphy-hub/struphy/blob/main/tutorials/tutorial_hagen_poiseuille_sph.ipynb)
- [`tutorial_dam_break_sph.ipynb`](https://github.com/struphy-hub/struphy/blob/main/tutorials/tutorial_dam_break_sph.ipynb)