...
**Substitute the particle approximation** $\rho^{\mathrm{vol}}\approx\rho_N = \sum_{i=1}^N w_i\,\delta(\mathbf{x}-\mathbf{x}^i(t))$
...
where $\rho_i := \rho_{h,N}(t,\mathbf{x}^i) = \sum_j w_j\,W_h(\mathbf{x}^i-\mathbf{x}^j)$.

- **External force**: $-\nabla V/m$ (gravity, external pressure gradient, …)
- **Pressure / viscous term**: $-\kappa\sum_j w_j\!\left(\tfrac{1}{\rho_i}+\tfrac{1}{\rho_j}\right)\nabla W_h$ — the standard _SPH pressure force_, here **derived from a variational principle**
